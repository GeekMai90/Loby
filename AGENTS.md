...
- GitHub 公开仓库 `GeekMai90/Loby` 仍是源码与三平台桌面 Release 的唯一事实来源；正式版本使用 `v<version>` tag 与 `落笔 <version>` 标题，先在 GitHub 建立并校验同提交草稿，再完成 macOS/Windows 的 Gitee 镜像验收，最后公开 GitHub Release。dry-run 让质量门禁与 macOS Apple Silicon DMG、Windows x64 NSIS、Linux x64 AppImage 原生构建并行执行，正式模式按 Run ID 复验并提升同提交资产，不重复构建。
