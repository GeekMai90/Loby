...
秘密不得进入写作库、浏览器存储、日志或前端响应。provider 输入在本模块边界校验，前端只消费稳定 command 与进度 event。设置目录用本地凭证存在性即时恢复“已添加”状态，不因进入页面自动访问 GitHub；用户显式刷新、打开仓库设置或真实发布时才访问远端，其中设置仓库查询可以使用短期快照，真实发布必须在打包内容前针对目标仓库重新验证写权限。
