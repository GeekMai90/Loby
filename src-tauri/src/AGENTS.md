...
models.rs - 跨 command 的序列化模型，包括文稿收藏元数据、项目发布目标绑定、带输入指纹的 GitHub/微信公众号发布记录、Provider 能力、Agent Skill、Agent Event Protocol v2 生命周期、stream 指标与图片产物
fs_paths.rs - 通用安全路径与文件名能力
markdown.rs - Markdown/frontmatter 解析与渲染，包括 `loby.favorite` 文稿收藏字段
