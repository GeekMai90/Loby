...
公开仓库使用 GitHub-hosted Actions 为 push/Pull Request 提供只读质量门禁与跨运行 Rust 缓存；来自 fork 的 CI 不接触仓库 secret。桌面 Release 只允许 `workflow_dispatch`，dry-run 单次构建源码绑定资产，发布 job 以最小 `actions: read` / `contents: write` 权限和当前仓库短期 `GITHUB_TOKEN` 准备并校验同提交 GitHub 草稿，再以仅发布 job 可读的 `GITEE_RELEASE_TOKEN` 同步并验收 macOS/Windows 国内镜像，最后公开 GitHub Release。
