...
- Simplified the WeChat theme preview by removing its toolbar and zoom controls, centering the shared phone/desktop switcher above a fixed outer canvas, using the iPhone 17's 402-point width at 100% scale with an adaptive height, keeping article scrolling inside the device preview, using a solid window toolbar, and adding a tested rich-layout copy action beside Save Theme.
