...
- 优化文稿功能栏样式：统一查找标题与媒体、历史版本的顶部间距和行高，并将媒体无图片时的提示改为居中图标空状态。
- 移除查找模式标题按钮的背景，让标题与功能栏面板保持一致的轻量视觉层级。
- 在文稿卡片右键菜单中新增“查看媒体、查找替换、查看历史版本”分组，点击后可直达文稿功能栏对应视图。
- 增加文稿卡片右键“创建副本”：为当前文稿生成独立身份的副本并自动选中，副本同步生成可辨识的标题、首个 H1 与文件名，清空发布记录并可继续移动到其他项目。
- 调整文稿卡片右键菜单：将“同步到帮助中心”发布动作独立为单独分组。
