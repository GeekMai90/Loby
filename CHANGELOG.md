...
- 修复中文 Markdown 粗体在跨段落、同一行多个标记和未闭合标记场景下被错误配对，导致开启标记后意外插入空格、编辑器样式失效的问题。
- 统一编辑器、微信公众号转换和预览的粗体兼容规则，保护代码、链接、Obsidian 引用与转义内容，并让预览和公众号发布读取编辑器最新未保存正文。

## 0.5.0 - 2026-08-16

- 优化微信公众号预览复制：现在会按“摘要 → 标题 → 公众号排版正文”的顺序写入三条独立剪贴板记录，正文保留 WebKit 富文本和内联样式，便于通过 HapiGo 逐条粘贴到公众号编辑器。
- 移除 0.4.7 引入的中文输入法斜杠键归一：斜线菜单只识别半角 `/`，不再按物理键位改写输入法上屏的顿号。Windows 中文标点模式下 `/` 键上屏的顿号因此不再唤出斜线菜单，需切换到英文标点模式输入 `/`。
...
- 修复 AI 助手应用正文格式时 Markdown 标记不生效的问题：高亮统一使用 `==...==`，不再输出编辑器不支持的 `<mark>`，粗体等行内标记保持成对闭合并适配中文正文衔接。
- 修复 AI 正文审阅中点击“隐藏更改”仍显示修改后样式的问题：现在会切换到修改前 `baseBody` 的只读预览，同时保留原有文字差异显示链路，预览不会写回本地 Markdown。

## 0.4.2 - 2026-08-12

- 修复文稿列表在 Windows 等使用经典系统滚动条的环境中左右间距不一致的问题；覆盖式窄滚动条现在不会挤占卡片宽度，并与列表栏宽度调整手柄保持独立命中区域。
- 修复发布文稿时默认 AI Provider 尚未配置凭证仍会尝试生成摘要、继而阻断发布的问题；缺少凭证时会安全跳过自动摘要，并保留与当前 Provider 对应的错误提示。
- 修复文稿卡片在普通点击、轻微抖动或指针释放事件丢失后偶发进入拖拽状态的问题；拖拽现在只在主键保持按下并越过安全距离时激活，同时在捕获丢失、窗口失焦或页面隐藏时可靠清理会话。
...
- 修复 Markdown 导入时间在不同时区运行环境中发生偏移的问题；带时区的文稿元信息保留来源墙上时间，文件时间戳继续按当前系统本地时间显示。
- 修复 Windows 发布 runner 通过 Node 子进程启动 `npm.cmd` 时出现 `EINVAL`、导致 NSIS 构建无法开始的问题；单平台构建现在直接使用当前 Node 执行统一 Tauri 构建入口。
- 修正 Linux Tauri 2 updater 产物契约：AppImage 同时作为安装与更新资产，并使用实际生成的 `.AppImage.sig`，不再等待不存在的 `.AppImage.tar.gz`。
...
- 增加文稿摘要元信息的 AI 生成：支持在属性面板手动生成；发布到各目标前若摘要为空，会自动补全后再继续发布，并复用设置中的默认 AI 配置。
- 优化文稿功能栏与右键菜单：新增媒体、查找替换和历史版本快捷入口，调整发布动作分组，并支持创建独立副本。
- 增加 Hugo 项目级批量发布，支持将项目内容原子化提交到 GitHub。
- 稳定微信公众号发布输出，减少发布内容与预期格式不一致的问题。
