...
- 项目位于 `projects/项目名/`。`project.toml` 只保存项目身份与状态、分组、纯项目目标、按项目隔离的文稿自定义属性定义、发布配置/清单和导出记录；文章标签、摘要和目标字数归每篇 Markdown 文稿。`README.md` 提供离开应用后的可读说明。
...
应用拥有的字段包括稳定 ID、标题、状态、标签、目标字数、摘要、创建/更新时间、归档/完成时间与自定义属性。未知的合法自定义属性必须尽可能保留；格式损坏的 frontmatter 不得被静默丢弃为“空元数据”。
