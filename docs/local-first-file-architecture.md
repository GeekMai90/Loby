...
- 项目位于 `projects/项目名/`。`project.toml` 只保存项目身份与状态、分组、纯项目目标、按项目隔离的文稿自定义属性定义、发布清单和导出记录；文章标签、`description` 和目标字数归每篇 Markdown 文稿，应用级发布目标归平台 app-config。`README.md` 提供离开应用后的可读说明。
