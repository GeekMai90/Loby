...
src-tauri/src/
  ...
  app.rs                   Tauri builder、state、平台菜单与 command 注册
...
- `app.rs` 注册官方 updater/process plugins；renderer 只能获得签名包检查、安装和重启权限。更新源固定为公开 `GeekMai90/Loby-Releases` 的静态 `latest.json`，由 Tauri 从完整的 macOS/Windows/Linux manifest 中选择当前平台，不引入自建服务，也不让 updater 接触写作库。
