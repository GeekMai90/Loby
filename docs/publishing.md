...
GitHub Device Flow 只把一次性用户码与本地流程 ID 展示给 renderer，GitHub `device_code`、access token 与 refresh token 始终停留在 Rust；后两者保存在 app-config secret store。用户 access token 失效时原生层使用 refresh token 自动轮换；项目设置通过 GitHub App installation API 查询当前账号获准且可写的所有仓库，不允许 renderer 自建第二套 OAuth 或 token 存储。
...
项目 GitHub 发布与墨问共享打字机、进度条和成功态视觉；进度值由 preparing、packaging、committing、finished 等原生事件映射。成功回写 `loby.blog` 元数据时必须保持当前 Dialog 的 success 状态，直到用户点击“完成”，不得因父级 sheet 更新重新显示发布确认表单。
