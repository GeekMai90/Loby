...
“设置 → 发布 → 发布目标”可添加微信公众号：AppID 保存在 app-config，AppSecret 只进入 native secret store，renderer 只读取是否已配置。打开设置和保存配置不访问微信；行菜单“验证连接”或公众号预览中的“推送到草稿箱”才获取 access token。Loby 不提供公共固定出口、代理或正式发布能力，用户负责把当前网络的公网 IP 加入公众号后台白名单；微信返回 `40164` 时，界面必须显示响应中的当前公网 IP 与“设置与开发 → 基本配置 → IP 白名单”路径。
