...
- If an AI edit is applied to a sheet that is no longer active by the time the CLI returns, Nibva should return to that target sheet and keep the AI panel open so the writer immediately sees the operation card and editor diff controls.
- If the target sheet body no longer matches the body that was sent to AI, Nibva should cancel automatic application and show an operation-card error instead of overwriting writer edits made while AI was running. Because nothing was applied, the card should offer review and dismiss controls instead of undo.
...
For structured Nibva actions:

- AI may propose app actions with `nibva-action` JSON blocks.
- Supported proposed actions are `createSheet`, `insertText`, `insertImage`, and `saveExport`.
- Dedicated block names `nibva-create-sheet`, `nibva-insert-text`, `nibva-insert-image`, and `nibva-save-export` are also accepted.
...
- Action cards are bound to the project and, for sheet insertion actions, the sheet that was active when AI generated the proposal. If the writer switches elsewhere before execution or retry, Nibva should show a card warning, disable execution, offer a control to return to the recorded target when it still exists, and refuse to apply the action until the original target is active again.
...
- If Nibva reloads while an action is `applying`, the loaded conversation normalizes that action to `failed` with a visible recovery message, so the user can inspect local state and retry instead of being stuck.
...
- Execution buttons call Nibva's existing app logic for sheet creation, image insertion, or export saving instead of letting the CLI write arbitrary files.
- `insertText` is for small, user-approved Markdown insertions such as transition paragraphs, outline fragments, intros, endings, and publishing notes. It should not be used for whole-document rewrites; use `nibva-change` for reviewable body edits.
...
- Before applying `insertText` or `insertImage` through the editor, Nibva should verify that the live editor document still matches the target sheet body. If the editor is stale or belongs to a different document, fail the action visibly instead of writing into an outdated view.
...
- Selected-text context follows the live editor selection: when the editor selection is cleared, Nibva clears the mounted selection context so stale selected text is not sent to AI.
...
- Older saved messages may not have an explicit context source marker; Nibva should infer document previews as live references and selection previews as snapshots.
...
## Nibva Operating Context

Nibva should treat Codex CLI as the AI engine, not as an unconstrained file editor. Every turn should give the CLI enough app-specific context to behave like a Nibva writing assistant:
...
- A warning not to directly edit `.nibva/` indexes, AI conversation files, caches, or trash
- The reviewable `nibva-change` protocol for body edits
- The proposed `nibva-action` protocol for new sheets, text insertion, image insertion, and exports
...
Task-specific Codex skills remain separate. The operating context teaches Codex how Nibva works; skills teach Codex how to perform a particular workflow such as headline generation, polishing, cover prompts, WeChat formatting, or Xiaohongshu repurposing.
