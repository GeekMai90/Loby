...
该方案复用现有构建与签名资产，不增加服务器、OSS、CDN 或备案成本；国内用户获得一个公开静态备用下载源，GitHub 仍保留完整回退能力。把 Gitee 上传从 GitHub-hosted Runner 迁移到可信本机，避开已验证的跨境大文件上传超时，同时不把 Gitee token 暴露给 GitHub Actions。代价是正式发布多一个本机接管步骤；镜像失败时 GitHub 保持草稿，修复网络后可只重试本机镜像，不能把未验收的 Gitee 清单视为可用。
