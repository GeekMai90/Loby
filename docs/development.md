...
执行顺序：进入目录先读最近的 L2，修改文件前读 L3；完成后按 L3 → L2 → L1 回环检查。新建有真实责任的模块时播种 L2；手写的运行源码、UI primitives、样式源、构建配置与仓库脚本必须播种一份且仅一份 L3。测试文件可在其职责需要长期解释时保留 L3，但不做机械覆盖；生成声明、生成产物、依赖缓存、测试 fixture 与临时 QA 证据不纳入头部契约。禁止为了“覆盖率”批量插入空洞模板。

`npm run check:architecture` 必须同时证明：L2 的固定 protocol、父级链接和直接成员清单有效；受管源码的 L3 四字段完整且不存在重复头部。发现重复契约时保留职责准确的一份，不允许用第二套泛化模板覆盖真实模块位置。
