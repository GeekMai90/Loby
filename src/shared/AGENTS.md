...
lib/platform.ts - Tauri 与浏览器平台边界检测；集中 Windows 桌面运行时识别和文件管理器平台文案，供应用窗口外壳与跨平台菜单使用
lib/platform.test.ts - Windows/macOS/Linux 文件管理器名称和桌面运行时识别的纯规则回归边界
lib/windowsWindow.ts - Windows 窗口状态恢复后的普通窗口工作区修正；只处理越过任务栏的已保存边界，最大化与还原继续使用 Tauri 原生语义
...
hooks/useWindowChrome.test.ts - 窗口 chrome 的拖拽与双击最大化回归边界，覆盖原生拖拽吞掉 `dblclick` 的时序与 Portal 交互浮层隔离
