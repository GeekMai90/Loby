/**
 * [INPUT]: 依赖 React、motion、Highlight、Slot 与 shared getStrictContext
 * [OUTPUT]: 对外提供 Tabs 状态容器、列表、触发器、滑动高亮、动画内容面板及相关类型
 * [POS]: components/animate-ui 的 Tabs 行为 primitive；统一选中状态、可变高度测量和内容横向过渡
...
import { getStrictContext } from "@/shared/lib/getStrictContext";
...
function TabsHighlight({ transition = { type: "spring", stiffness: 200, damping: 25 }, ...props }: TabsHighlightProps) {
  ...
  return <Highlight data-slot="tabs-highlight" controlledItems value={activeValue} transition={transition} click={false} {...props} />;
}
