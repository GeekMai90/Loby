...
function SlidingNumberRoller({ prevValue, value, place, transition, delay = 0 }: SlidingNumberRollerProps) {
...
function SlidingNumber({
  ...
  const [previousNumber, setPreviousNumber] = React.useState<number>(initiallyStable ? initialNumeric : 0);
  ...
  const [effectiveNumber, setEffectiveNumber] = React.useState<number>(initiallyStable ? initialNumeric : 0);
  ...
  }, [hasAnimated, springVal, isInView, number, decimalPlaces, onNumberChange, effectiveNumber, initiallyStable, initialNumeric]);
  ...
  const finalIntLength = padStart ? Math.max(Math.floor(Math.abs(number)).toString().length, newIntStrRaw.length) : newIntStrRaw.length;
  ...
  const adjustedPrevInt = React.useMemo(() => {
    return prevIntStr.length > finalIntLength ? prevIntStr.slice(-finalIntLength) : prevIntStr.padStart(finalIntLength, "0");
  ...
  React.useEffect(() => {
    if (isInView || initiallyStable) {
      setPreviousNumber(effectiveNumber);
    }
  ...
  return (
    ...
    >
      ...
      {intPlaces.map((place, idx) => {
        ...
        const isSeparatorPosition = typeof thousandSeparator !== "undefined" && digitsToRight > 0 && digitsToRight % 3 === 0;
