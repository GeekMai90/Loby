...
dialog.tsx - 通用 Dialog primitives，统一无模糊半透明 scrim、焦点接管、默认居中/显式自定义定位、close 与同表面 footer
dialog.test.ts - Dialog 表面、定位声明、footer 排列与自动焦点契约回归
...
select.tsx - Radix Select 的 13px trigger/条目、compact/default/wide/full/fit 五档 trigger 宽度、可选等宽/内容自适应/固定 popup 宽度、滚动控件与紧凑条目几何
select.test.ts - Select 13px 字号、语义宽度映射、Trigger/Content 默认等宽、内容自适应、独立宽度与超长条目截断契约回归
