...
WebView 文档根不得声明固定最小高度；macOS/Linux 的窗口下限由默认 Tauri 配置承担，Windows 的实际显示器工作区优先于设计高度，避免高 DPI 缩放后内容越过任务栏。`.loby-window` 可以保留非 Windows 入口的设计下限，但 Windows 外壳必须在自己的父子作用域内明确允许收缩。
...
shell.css - 桌面窗口、Windows 自定义标题栏及 Portal 模态安全区、三栏工作区、resizer、左缘悬停导航与 inspector 几何；文稿列表滚动条与列表栏宽度拖拽手柄共享非重叠安全边界，窗口外沿的圆角与描边归系统所有，`.loby-window` 不再自绘
left-workspace-glass.css - 左侧工作区液态玻璃材质、临时悬浮显隐及透明降级
library-rail.css - 写作文件夹导航的拖放、打开进度与重排反馈
writing-goals.css - 写作活动热力与现役项目目标进度动画
sheet-row.css - Bear 式文稿卡片内容层级、置顶标记、首图缩略图、组合选中、焦点分离与拖放状态
