/**
 * [INPUT]: 依赖 @uiw/react-codemirror、CodeMirror 6、Tauri API、React 运行时、编辑器模块、shared 公共契约
 * [OUTPUT]: 对外提供 ZenModeWindow
...
import { EditorSelectionToolbar } from "@/features/editor/components/EditorSelectionToolbar";
import { WindowControls } from "@/shared/components/WindowControls";
import { ZenModeControlMenu } from "@/features/zen-mode/components/ZenModeControlMenu";
...
import {
  resolveEditorSelectionToolbarPosition,
  type EditorSelectionToolbarPosition,
} from "@/features/editor/model/editorSelectionToolbarPosition";
...
import { safeVisiblePathSegment } from "@/features/library/model/projectModel";
import {
...
import { ZenSoundscape } from "@/features/zen-mode/model/zenSound";
import { loadAgentSettings } from "@/features/assistant/model/agentSettings";
import { countWords } from "@/shared/lib/text";
import { WritingGoalProgress } from "@/features/writing-activity/components/WritingGoalProgress";
      ...
      <section ref={writingPanelRef} className="zen-writing-panel" aria-label={`禅模式：${session.sheet.title}`} style={editorStyle}>
        <header className="zen-writing-header" data-tauri-drag-region onDoubleClick={toggleMaximizeWindow}>
          ...
          <div className="zen-header-status-group">
            <WritingGoalProgress sheetId={session.sheet.id} wordCount={countWords(body)} targetWords={session.sheet.targetWords} />
