...
斜线菜单的触发字符由 `shared/lib/slashTrigger.ts` 唯一定义，AI composer 复用同一边界：半角 `/`、Windows 中文输入法中文标点模式上屏的顿号 `、` 与全角 `／` 等价。中文标点由 IME 直接上屏，编辑器只看得到最终字符，因此不得改为按物理键位或运行平台分支识别；误触由「只在行首或空白之后触发」排除——真正的中文顿号总是紧跟在字符之后。
