...
斜线菜单的触发字符由 `shared/lib/slashTrigger.ts` 唯一定义，AI composer 复用同一边界，且只有半角 `/`。Windows 中文输入法把物理 `/` 键上屏为顿号 `、` 或全角 `／` 的问题在输入层解决：`model/editorImeSlashKey.ts` 装配在 slashMenuExtension 上游，按 keydown 的物理 `code` 把 Slash 键上屏的替身在事务落库前改写回 `/`。顿号的标准键位是 `\`，因此绝不能在字符层把 `、` 与 `/` 等价——那会让顿号无法输入；同样不得按运行平台分支识别，键位判定在所有平台上都是同一套。归一失效时必须退化为「顿号照常输入、菜单不弹」，不能反向牺牲中文标点。
