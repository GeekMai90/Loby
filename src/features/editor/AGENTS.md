...
编辑器顶栏的图片入口位于文稿信息按钮左侧，只在当前 CodeMirror 编辑器拥有焦点时启用；点击通过 App 复用 features/media 的图片来源 Dialog，按钮按下不会抢走编辑器焦点，因此本地/Unsplash 图片仍插入当前光标或选区。
...
编辑器 Markdown 组合键只保留粗体、斜体、链接与行内代码；标题、列表、任务和引用继续通过斜线菜单等显式入口调用格式化能力。编辑器行内格式契约由 `model/editorMarkdownLanguage.ts` 与装饰层共同实现：粗体 `**...**`、斜体 `*...*`、下划线 `~...~`、删除线 `~~...~~`、高亮 `==...==`；HTML `<mark>` 不属于正文格式协议，中文标点结尾的粗体紧接中文正文时仍需被识别。启用“规范 Markdown 标记”时，`shared/lib/cjkStrongEmphasis.ts` 以逐行、非交叉 delimiter 配对统一清理粗体内侧的半角、全角、不换行与零宽空白，并为中文标点关闭边界补充可移植空格；未闭合标记、上一段或同一行相邻粗体不得污染下一处开启标记。块标记格式化必须保护行首粗体/斜体、正负数、代码、链接、Obsidian 引用、硬换行与转义内容。CodeMirror 默认的 `Mod-/` 注释动作必须排除，始终将该组合键交给应用切换快捷键面板。
...
斜线菜单的“图片”入口先交给 `features/media` 选择本地或 Unsplash 来源；本地选择继续打开系统文件选择器，在线选择必须在 native 下载并按用户确认的比例裁剪后写入 `assets/images`，再由 `useEditorImages` 插入同一套标准 Markdown 引用，编辑器正文不保存远程图片地址。
