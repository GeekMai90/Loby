 ...
 * [OUTPUT]: 对外提供 buildLobyOperatingContext，声明 Loby Markdown 行内格式契约、结构化文稿提案工具、精确插入策略、标准图片路径示例与本地写作边界
...
export function buildLobyOperatingContext({
  ...
  return [
    ...
    "- Markdown 文件是内容源头，必须保持可被系统文件管理器、Obsidian 和普通 Markdown 工具读取。",
    ...
    "",
    "编辑器 Markdown 格式契约（修改正文时必须遵守）：",
    "- 粗体使用 `**文字**`，斜体使用 `*文字*`，下划线使用 `~文字~`，删除线使用 `~~文字~~`。",
    "- 高亮必须使用 `==重点文字==`；禁止使用 `<mark>重点文字</mark>`、`::重点文字::` 或其他 HTML/旧方言写法。",
    "- 行内代码使用 `代码`，链接使用 `[链接文字](https://example.com)`；标题、引用、列表和任务使用标准 Markdown。",
    "- 不要使用 `<strong>`、`<em>`、`<u>`、`<s>` 等 HTML 标签代替 Markdown 格式；Markdown 正文需要能在 Loby 编辑器中即时显示。",
    "- 行内格式必须成对、尽量保持在同一行；格式结束后如果紧接中文正文，关闭标记后保留一个空格，例如 `**“注意力 > 时间 > 金钱”** 这个价值观`，不要写成 `**“注意力 > 时间 > 金钱”**这个价值观`。",
    "",
    "结构化文稿提案：",
