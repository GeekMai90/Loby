...
export function buildLobyOperatingContext({
  ...
  return [
    ...
    "- 纯生成、分析、建议和预览不需要动作确认；只有下一步会修改持久化文稿、创建文稿或写出导出文件时才输出 action。",
    "- 创建新文稿、保存导出、插入文字或图片等动作，先给必要说明；不要直接写文件。",
    "- 当用户明确要求创建文稿、插入当前文本片段、插入图片引用或保存导出时，必须追加 `loby-action` JSON 代码块。落笔会在消息流中完整展示 payload 成果，并在下方显示写入确认条。",
