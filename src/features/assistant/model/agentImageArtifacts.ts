 ...
 * [OUTPUT]: 对外提供单轮及跨轮生成图片来源关联、运行产物收集与 run/action 展示去重
 * [POS]: AI 助手图片成果的身份归一化边界，让缓存源产物在确认前跨消息保持身份，确认后再交给持久化路径
...
export function linkConversationGeneratedImageActions(messages: ChatMessage[]): ChatMessage[] {
  ...
  return messages.map((message) => {
    ...
    return message.actions ? { ...message, actions: linkGeneratedImageActions(message.actions, precedingActivities) } : message;
  });
}
