...
`model/imageSearchQuery.ts` 通过独立 native 封面检索词 command，把当前文稿的标题、摘要和首尾有界正文投影为 2 到 5 个单词的可编辑英文 Unsplash 搜索词；同时通过隔离的翻译 command 将用户输入的中文搜索词转换为同一格式。它兼容 JSON、标签、围栏等常见模型返回，但拒绝中文、解释、单词或过长结果，不复用中文摘要系统指令，也不写入正文、图片资源或文稿元数据。

`model/agentConnectionDirectory.ts` 是已配置 AI 连接的共享投影边界；除加载连接及模型目录外，还提供不触发模型请求的轻量连接探测，供写作设置在开启 AI 推荐前复用同一凭证事实来源。

作者控制权是硬边界：AI 变更必须可审阅、可拒绝、可撤销；显示/隐藏更改在当前 proposedBody 与修改前 baseBody 之间做只读视觉切换，不把预览正文写回文稿；消息历史、运行时与编辑器 diff 不得混为一个状态机。feature 可以消费编辑器和写作库的稳定模型，但不得接管其持久化所有权。任何 AI 插入、接受或恢复在校验和创建版本快照时都必须读取编辑器实时正文；React 中稍旧的文稿对象不能成为恢复点，避免撤回动作删除作者刚输入的内容。

结构化 `propose_document_change.proposedBody` 与发送时的 `baseBody` 是正文审阅的事实来源。模型提供的 `changes` 只有在能够完整重建 `proposedBody` 时才可作为精细 diff；描述性、遗漏或与最终正文不一致的变更清单必须退回到两版正文的 Myers 最小字符差异，并记录 base/proposed 双版本偏移与邻近上下文。AI 修改正文时必须遵守 Loby Markdown 行内格式契约：高亮使用 `==...==`，不能写 `<mark>...</mark>`；粗体等标记必须成对，紧接中文正文时在关闭标记后保留空格。旧会话的 `loby-change` / `loby-action` 只在展示边界兼容解析，新请求不得继续要求模型输出 Markdown 协议块。空白行不得作为自然段错配锚点，已持久化但无法重建最终正文的旧变更也必须在展示边界重新计算。
...
composer 中由 `/` 与 `@` 触发的输入建议统一复用 `components/ui/suggestion-menu.tsx`，其中 `/` 的触发边界与编辑器斜线菜单共用 `shared/lib/slashTrigger.ts`，不在 composer 内另写正则；保持与 DropdownMenu 一致的实体材质、菜单几何和选中状态；文本框以 combobox 暴露展开状态、受控 listbox 与当前 active option，键盘导航仍由 composer 状态机持有。
