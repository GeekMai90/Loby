...
主助手 thread 默认关闭全局 Codex Memory、插件/Apps 与自动 Skill 目录注入；用户在 composer 显式选择的 Skill 由落笔读取并按 turn 注入，只有插件缓存中的 Skill 才按需恢复插件能力。落笔操作协议保持单一紧凑来源，禁止在上下文装配层重复动作 schema、路径和示例。
