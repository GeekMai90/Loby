...
composer 中由 `/` 与 `@` 触发的输入建议统一复用 `components/ui/suggestion-menu.tsx`，其中 `/` 的触发字符集与编辑器斜线菜单共用 `shared/lib/slashTrigger.ts`，不在 composer 内另写正则；保持与 DropdownMenu 一致的实体材质、菜单几何和选中状态；文本框以 combobox 暴露展开状态、受控 listbox 与当前 active option，键盘导航仍由 composer 状态机持有。
