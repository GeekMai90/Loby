...
export function AssistantComposerShell({ className, glowActive = false, children, ...props }: AssistantComposerShellProps) {
  return (
    <form
      ...
      className={cn(
        "relative mx-[var(--assistant-panel-gutter)] mb-1 flex shrink-0 flex-col gap-1.5 rounded-2xl border border-[var(--separator)] bg-[var(--assistant-composer-background)] p-2.5 focus-within:border-[var(--separator)] focus-within:ring-0",
        className,
      )}
      {...props}
    >
