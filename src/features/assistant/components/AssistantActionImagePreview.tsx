 ...
 * [OUTPUT]: 对外提供 AssistantActionImagePreview，完整显示 action 生成图片并调用桌面系统预览本地原图
...
export function AssistantActionImagePreview({ preview }: { preview: InsertImageActionPreview }) {
  ...
  const supportsNativePreview = isAbsoluteLocalPath(preview.sourcePath) || /^https?:\/\//i.test(preview.sourcePath);
