...
AI 助手展示形态不在设置 Dialog 维护；其唯一持久入口是助手自身更多菜单中的“固定到侧边”。设置页维护发送快捷键、quick prompts、当前写作库 Skill 的导入/诊断/启停/删除、Provider、应用内凭证、ChatGPT Device OAuth、联网搜索与 MCP server；Skill 设置只调用 native Skill Store，不复制文件校验；敏感表单只持有未提交草稿，OAuth 界面只接收设备码和去敏账号状态，保存后不得从 native 读回明文。
