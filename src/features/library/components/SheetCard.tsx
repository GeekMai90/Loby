 ...
 * [OUTPUT]: 对外提供 Bear 式 SheetCard 展示组件与 SheetCardImage 契约，并在列表搜索时展示命中行、高亮文本、上下文标签与置顶标记
...
export function SheetCard({ sheet, metaLabel, image, search = "" }: SheetCardProps) {
  ...
  return (
    <div className="sheet-card flex min-h-0 flex-1 flex-col">
      ...
      <small className="sheet-row-meta sheet-card-meta truncate">
        ...
        {getSheetMetaText(sheet, metaLabel, now)}
      </small>
    </div>
  );
}
