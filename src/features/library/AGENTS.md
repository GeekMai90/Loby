...
用户新建项目只建立通用空容器，不预生成提纲、素材库或发布检查等内容；首次建库生成的“落笔指南”是产品内置教程，不属于项目模板。项目的文稿属性定义默认只含锁定结构的“目标字数”，其默认值仅供以后新建文稿在创建时继承，不批量回写已有文稿；其他文稿系统值仍由每篇文稿直接持有。文稿自定义属性可以按项目隔离存储定义，但归编辑器的文稿属性管理；项目模型只拥有项目身份、分组、纯项目目标、文稿创建默认值与生命周期，不拥有应用级发布配置。文稿没有独立的系统完成状态，项目篇数目标直接派生自未归档文稿数量。
