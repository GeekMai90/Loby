...
文稿收藏是 `WritingSheet` 自身的轻量元数据，写入 Markdown 的 `loby.favorite` 并由项目索引冗余恢复；一级导航“收藏”跨收件箱、随手记与普通项目筛选全部已收藏文稿，包括之后归档但仍保留收藏标记的文稿，不创建目录、不改变项目归属，也不得借收藏动作改写正文更新时间或排序语义。单篇右键菜单负责切换收藏与取消收藏，多选不隐式批量收藏。
