...
`components/SheetCard.tsx` 是 Bear 式文稿摘要的纯展示边界：顶部固定保留两行文字区，短标题使用“一行标题 + 一行正文”，长标题换行后独占两行并隐藏正文；列表搜索时标题保持在标题行高亮，正文命中则把命中所在行切换到正文位置并高亮；无图文稿与空文稿保持紧凑，底部显示“相对时间/日期 · 上下文标签”，跨项目筛选使用所属项目，真实项目详情使用文稿所在分组，有图文稿只增加首张可解析图片的固定缩略图。卡片不拥有选择、拖拽或持久化。
...
文稿排序标题、创建/内容更新时间键与固定查询词的搜索命中按 `WritingSheet` 对象身份弱缓存；`updatedAt` 只表达标题或正文等内容变化，项目/分组移动、属性、归档、收藏、置顶与发布记录等元信息变化不得改写它。App 更新正文必须保留其他文稿对象引用，使单文稿提交不会再次扫描所有未变化正文或重复解析日期。对象变化即自然失效，禁止用 sheet id 缓存而遗漏元数据更新。
...
`model/githubRepositoryStats.ts` 负责按需读取公开 GitHub 仓库的 Star 数；优先使用 GitHub API，匿名接口限流或异常时回退 Shields 公共缓存接口，仅缓存成功结果，双重失败返回空值，由 `LibraryRailFooter` 投影到开源二级菜单，不改变写作库事实源。
...
Markdown 导入先经 native 递归扫描和 Obsidian Vault/附件识别，再由 `model/importMarkdown.ts` 按目标项目映射元信息、一级分组和标准图片引用；带时区的外部元信息保留来源文档写下的墙上时间，文件系统时间戳才按当前运行环境的本地时间格式化，测试不得绑定开发机时区。文件菜单、项目右键与空状态只调用 `hooks/useMarkdownImport.ts`，不得各自创建平行导入流程。收件箱递归接收但保持扁平，普通项目把多级来源目录展平为分组；外部身份和未匹配字段不进入文稿模型。
...
导航栏与文稿列表分别保留 selection，focus 只决定哪一栏显示 active 视觉，不得清空另一栏的选择。项目、分组与分类导航只改变列表浏览范围，不自动写入当前编辑文稿；当前文稿暂时不在列表中时仍保留编辑器内容，只有用户主动点击文稿才切换。编辑器获得焦点时两栏进入 inactive-selection；具体颜色只由语义 Token 和设计文档定义。
...
导航栏底部工具条左侧保留设置与主题切换，最右侧以圆圈问号打开“新功能、键盘快捷键、帮助、开源、检查更新”菜单，其中“开源”展开无前置图标的 GitHub 与 Gitee 二级入口；帮助菜单打开时按需读取 GitHub 公开仓库的 Star 数，成功后在 GitHub 项目入口右侧显示带本地化千位分隔的数量，失败不阻断菜单且不伪造数字。检查更新期间不展示更新卡片，确认有新版本后才在工具条上方显示无边框的简洁提醒卡片，鼠标悬停卡片时激活共享 `BorderGlow` 旋转边缘光效，卡片同时使用庆祝图标、固定说明文案、可在“立即更新→下载进度→重启安装”之间切换的全宽操作按钮和悬停显现的无背景关闭按钮；关闭时卡片向下收进工具条分隔线，同时该位置临时替换为主题色下载按钮。library 只接收 app 注入的状态和动作，不直接依赖 updater、不持有下载或安装生命周期。
