...
启动恢复同时发起默认目录、当前写作库与 AI 会话读取；文稿/项目状态一旦可用即可进入 React，互不依赖的本地读取不得人为串行。切换写作库时，文库与会话读取也必须并行，但 flush 当前待写内容仍是所有目标读取之前的硬边界。

本地目录与 Markdown 是事实来源。新建、导入与 AI 创建文稿统一消费 `model/documentId.ts` 的 `sheet-` 加 26 位小写 Base32 身份；旧 Markdown 只在用户主动重建索引时由 native 迁移，普通外部刷新不得静默改写身份。切换写作文件夹时，native 必须接受已有落笔结构或初始化用户明确选择的空文件夹，拒绝带普通内容的非落笔目录；准备完成后先保存当前待写内容，再切换 registry、项目、会话与选择，误选目录不得注册。高频正文保存只发送目标文稿与最小项目路径上下文，metadata-only 索引不得携带当前正文副本或反向覆盖 Markdown；创建、删除、移动与项目元数据变化继续走结构保存。可跨项目发布的 GitHub 目标统一归应用级 publishing registry；项目只可拥有自身帮助中心的非敏感仓库引用与分组目录映射，不拥有 GitHub 凭证，其他发布时仍只提供文稿组织和本地图片路径上下文。registry 的删除和显示名修改不得触碰实际文件夹；持久化、外部刷新与选择修复的时序只能在集成覆盖保护下调整。
