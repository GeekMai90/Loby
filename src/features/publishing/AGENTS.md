...
公众号主题工作室只持有主题 manifest、revision、主题作用域会话和主题结果协议；主题目录代表色由合并后的 `baseStyle.colors` 派生，AI patch 不直接拥有 `swatches`；AI 连接目录、Composer、会话上下文规划、stream 取消/引导、`assistant-ui` top-anchor 消息定位与通用图片/文档附件输入复用 `features/assistant` 的共享能力。主题助手仍以 `wechat-theme-designer` Skill 作为领域适配，不得把主助手的正文 proposal/action 状态机引入发布 feature，也不得为附件类型另起输入链路。
