...
<member>
App.tsx - 主窗口协调器，持有跨功能状态、首次启动与帮助菜单欢迎界面、签名桌面更新和安装前写作队列 flush、应用设置、写作目标映射、标准 Markdown 图片写入、原生菜单、快捷键、即时列表选择与可中断文稿导航、文稿收藏/置顶/创建副本菜单动作、异步 CodeMirror、实时正文到排版/替换/历史保护/耐久化的权威读取、AI 形态、应用级发布目标、统一项目发布/单篇帮助中心入口、Markdown 导入、持久化所有权及主要界面组合；项目浏览上下文与当前编辑文稿选择分离
